...
EXPECTED_OUTPUT="WARNING: NumericalStabilitySanitizer: NaN detected"
...
if [[ "$(trim "$OUTPUT")" == *"$(trim "$EXPECTED_OUTPUT")"* ]]; then
  echo "✅ NSan output contains expected string."
else
  echo "❌ NSan output does not contain expected string."
  echo
  echo "---- Expected ----"
  printf '%s\n' "$EXPECTED_OUTPUT"
  echo "---- Got ----"

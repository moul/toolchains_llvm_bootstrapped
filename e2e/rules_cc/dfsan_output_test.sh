...
EXPECTED_OUTPUT="WARNING: DataFlowSanitizer: call to uninstrumented function dfsan_e2e_probe"
...
if [[ "$(trim "$OUTPUT")" == *"$(trim "$EXPECTED_OUTPUT")"* ]]; then
  echo "✅ DFSan output contains expected string."
else
  echo "❌ DFSan output does not contain expected string."
  echo
  echo "---- Expected ----"
  printf '%s\n' "$EXPECTED_OUTPUT"
  echo "---- Got ----"

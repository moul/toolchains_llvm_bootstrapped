...
EXPECTED_OUTPUT="ERROR: RealtimeSanitizer: unsafe-library-call"
...
if [[ "$(trim "$OUTPUT")" == *"$(trim "$EXPECTED_OUTPUT")"* ]]; then
  echo "✅ RTSan output contains expected string."
else
  echo "❌ RTSan output does not contain expected string."
  echo
  echo "---- Expected ----"
  printf '%s\n' "$EXPECTED_OUTPUT"
  echo "---- Got ----"
